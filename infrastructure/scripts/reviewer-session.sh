#!/bin/bash
# Complete Reviewer Bot Session Setup
# Everything needed for natural kinda-lang code review

set -e

echo "🔍 Starting Reviewer Bot Session..."

# Move to project directory
cd ~/kinda-lang

# Load and apply authentication
source ~/kinda-lang-agents/infrastructure/.env.local
export GH_TOKEN="$KINDA_REVIEWER_PAT"
export GH_REPO="kinda-lang-dev/kinda-lang"

# Set git identity
git config user.name "kinda-lang-reviewer" 
git config user.email "kinda-lang-reviewer@users.noreply.github.com"

# Activate virtual environment
source venv/bin/activate

echo "✅ Reviewer Bot Session Ready!"
echo "  GitHub: $(gh api user --jq .login)"
echo "  Python: $(python --version)" 
echo "  Branch: $(git branch --show-current)"
echo "  Location: $(pwd)"
echo ""
echo "🔍 Ready for code review tasks..."

# Run any command passed as arguments
if [ $# -gt 0 ]; then
    echo "Running: $*"
    exec "$@"
fi